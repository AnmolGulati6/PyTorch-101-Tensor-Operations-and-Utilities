...
def count_tensor_elements(x: Tensor) -> int:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    num_elements = torch.prod(torch.tensor(ten.size()))
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return num_elements
...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    mult10 = [i for i in range(start, stop) if i % 10 == 0]
    x = torch.tensor(mult10, dtype=torch.float64)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def slice_assignment_practice(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    grid = torch.tensor([[0, 1, 2, 2, 2, 2],[0, 1, 2, 2, 2, 2],[3, 4, 3, 4, 5, 5],[3, 4, 3, 4, 5, 5]], dtype=torch.int64)
    x[:4, :6] = grid
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def shuffle_cols(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    r, c = x.size()
    y = torch.zeros((r, 4), dtype=x.dtype, device=x.device)  
    y[:, 0:2] = x[:, 0:1]
    y[:, 2:4] = x[:, [2, 1]]
    ##########################################################################
    #                            END OF YOUR CODE                            #
...
def make_one_hot(x: List[int]) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    tens = torch.tensor(x, dtype=torch.long)
    y = torch.zeros((len(x), 1+torch.max(tens)), dtype=torch.float32)
    y[torch.arange(len(x)), tens] = 1
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def reshape_practice(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    inter = x.reshape(2,3,4)
    y = inter.transpose(0,1).reshape(3,8)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    mini = torch.argmin(x, dim = 1)
    y = x.clone()
    y[torch.arange(x.size(0)), min] = 0
...
def batched_matrix_multiply_loop(x: Tensor, y: Tensor) -> Tensor:
    ...
    a,b,c = x.shape
    d,e,f = y.shape
    z = torch.empty(a,b,f, dtype=x.dtype, device=x.device)
    for i in range(a):
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    sh = x.shape[0]
    y = (x-x.sum(dim=0)/sh)/torch.sqrt(((x - x.sum(dim=0)/sh) ** 2).sum(dim=0)/(sh-1))
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
